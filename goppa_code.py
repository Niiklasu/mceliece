...
class GoppaCode:
    def __init__(self, n: int, m: int, t: int):
        ...
        # support L is just list of elements, instead of scrambled, as it is recommended in many sources
        # I based this on Bernsteins statement that "[the] sequence [is] chosen as all the elements of F_2^m in lexicographic order"
        # in (2.) in this paper https://cr.yp.to/codes/goppalist-20081107.pdf
        # I tried to work with a scrambled list but that lead to some error down the line I couldn't figure out
        ...
        X = GF2m(
            [
                [
                    g_poly.coeffs[row - col] if 0 <= row - col < t else 0
                    for col in range(t)
                ]
                for row in range(t)
            ]
        ...
        Z = GF2m(
            [
                [g_poly(support[row]) ** -1 if col == row else 0 for col in range(n)]
                for row in range(n)
            ]
        )
        H = X @ Y @ Z
        ...
        H_bin = GF2(
            np.concatenate(
                [np.column_stack([elem.vector() for elem in row]) for row in H]
            )
        )
    ...
    def patterson_alg(self, received_message: np.ndarray) -> np.ndarray:
        ...
        inverse_polys = [
            inverse_mod_poly(
                Poly.Str(f"x - {self.supp[i]}", field=self.GF2m), self.g_poly
            )
    ...
    def decode(self, received_message: np.ndarray) -> np.ndarray:
        ...
        system_to_solve = np.append(
            self.G.T, fixed_message.reshape(len(fixed_message), 1), axis=1
        )
